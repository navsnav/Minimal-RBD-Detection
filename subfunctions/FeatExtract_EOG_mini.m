function [features, features_struct_30s,eog_data_signal,features_struct] = FeatExtract_EOG_mini(data_signal1, fs,epoch_time,feature_time,std_flag)
...
% end
if std_flag==1
    data_signal1 = data_signal1 - mean(data_signal1(:));
    data_signal1 = data_signal1./std(data_signal1(:));                     % standardizing
end
...
%%    
    PSDdelta = bandpower(x,fs,[0.5 4]);
    PSDtheta = bandpower(x,fs,[4 8]);
    PSDalpha = bandpower(x,fs,[8 13]);
    PSDbeta = bandpower(x,fs,[13 30]);
    
    PSDtotal = PSDdelta+PSDtheta+PSDalpha+PSDbeta;   
    
    RSPdelta = PSDdelta/PSDtotal;
    if isnan(RSPdelta)
        RSPdelta = 0;
    end    
    features_struct.EOG_RSPdelta(i,1) = RSPdelta;    
    
    RSPtheta = PSDtheta/PSDtotal;
    if isnan(RSPtheta)
        RSPtheta = 0;
    end      
    features_struct.EOG_RSPtheta(i,1) = RSPtheta;    
 
    RSPalpha = PSDalpha/PSDtotal;
    if isnan(RSPalpha)
        RSPalpha = 0;
    end       
    features_struct.EOG_RSPalpha(i,1) = RSPalpha; 
    
    RSPbeta = PSDbeta/PSDtotal;
    if isnan(RSPbeta)
        RSPbeta = 0;
    end         
    features_struct.EOG_RSPbeta(i,1) = RSPbeta;
...
%     [peakOne,peakTwo]=featureExtractTime_amplitude2peak(x,plotting); %yetton
    ...
    features_struct.EOG_MaxAutocorr(i,1) = peakXcor;  
    features_struct.EOG_MinAutocorr(i,1) = peakXcor;  
...
%     features_struct.EOG_NonLinear(i,1) = featureExtractTime_nonLinearEnergy(x);     %yetton
    eog_tkeo = TKEO(x);
    features_struct.EOG_TKEO(i,1) = mean(eog_tkeo(2:end-2));   
...
for x = 1:numel(Snames)    
    ...
    for y = 1:(size(feature_field,1)/num_miniepochs)
       if  strcmp(Snames{x},'MaxDx') || strcmp(Snames{x},'MaxCoastline') || strcmp(Snames{x},'MaxAutocorr') %Maximum feature within epoch
        temp_features(y,:) = max(feature_field((y-1)*num_miniepochs+1:y*num_miniepochs,:)); 
       elseif strcmp(Snames{x},'MinCoastline') || strcmp(Snames{x},'MinAutocorr')   %Min feature within epoch
