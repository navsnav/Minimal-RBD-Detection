function [features, features_struct_30s, EMG,features_struct] = FeatExtract_EMG_mini(data_signal1, fs,epoch_time, hyp, std_flag)
...
% fvtool(Notchfilter);
data_signal1 = filter(Notchfilter, data_signal1);
data_signal1 = filter(Notchfilter2, data_signal1);
end
...
% fvtool(LPfilter);
% fvt = fvtool(LPfilter);
data_signal1 = filtfilt(LPfilter.Numerator,1,data_signal1);

end

hp = fdesign.highpass('N,Fst,Fp',100,1,10,fs_prefilt);
HPfilter = design(hp);
dataNotchHPLP = filtfilt(HPfilter.Numerator,1,data_signal1);
...
EMG_avg_amp = zeros(floor(length(EMG_rect)/fs_emg),1);
...
%     if fs_prefilt < 100
%         gamma_pow = bandpower(pxx,fxx,[30 fs_prefilt/2],'psd');        
%     else
...
%     features_struct.EMG_Rel_Power(i,1) = EMG_rel_pow;
    ...
    PSDalpha = bandpower(x,fs,[10 13]);
    PSDbeta = bandpower(x,fs,[13 30]);
    PSDgamma = bandpower(x,fs,[30 45]);
    
    PSDtotal = PSDalpha+PSDbeta+PSDgamma;   
    
    RSPalpha = PSDalpha/PSDtotal;
    if isnan(RSPalpha)
        RSPalpha = 0;
    end       
    features_struct.EMG_RSPalpha(i,1) = RSPalpha; 
    
    RSPbeta = PSDbeta/PSDtotal;
    if isnan(RSPbeta)
        RSPbeta = 0;
    end         
    features_struct.EMG_RSPbeta(i,1) = RSPbeta;
    
    RSPgamma =  PSDgamma/PSDtotal;
     if isnan(RSPgamma)
        RSPgamma = 0;
    end     
    features_struct.EMG_RSPgamma(i,1) = RSPgamma;    
...
features_struct.EMG_Motor_Activity_Per = MAE_epoch(:,2);
...
for i = 1:num_epochs_10s
    ...
    if fs_prefilt < 100    
    pow = bandpower(pxx,fxx,[10 fs_prefilt/2],'psd');
    else
