function [All_Sleep, Sleep_Struct, All_Sleep_T] = ExtractFeatures_mat(dbpath,signals_for_processing,std_flag,filt_flag)
    ...
    for j = 1:length(signals_for_processing)
        ...
        switch char(signals_for_processing(j))
            case {'EEG'}
                ...
                if filt_flag
                    data_signal = filtfilt(b_band,1,data_signal);                    
                end
             ...
             case {'EOG2EEG'}
                EOG_CHAN = find(strcmp(cellstr(patinfo.chlabels), 'EOG'));
...
%                 epoch_time=30;
%                 fs=200;
                [ecg_feats, features_struct_30s,ecg_data_signal] = FeatExtract_HR_mat(data_signal, fs,epoch_time,std_flag,0,subject,dbpath,hyp); %Without RR averaging window
