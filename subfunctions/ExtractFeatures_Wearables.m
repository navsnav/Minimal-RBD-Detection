function [All_Sleep, Sleep_Struct, All_Sleep_T] = ExtractFeatures_Wearables(dbpath,signals_for_processing,std_flag,filt_flag)
...
%% Design Filters
    ...
    try
       ch_labels = patinfo.chlabels; 
    catch
       patinfo.chlabels = fieldnames(data); 
    end
...
%         data(any(any(isnan(data),3),2),:,:) = [];
        ...
        switch char(signals_for_processing(j))
            case {'EEG'}
                feature_time = 10;
                EEG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'C4'));                
                EEG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'A1'));
                if isempty(EEG_CHAN2) && isempty(EEG_CHAN1)
                    EEG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'Fp1'));                
                    EEG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'Fp2'));
                end

                if ~isempty(EEG_CHAN1) && ~isempty(EEG_CHAN2)
                       disp(['EEG Signals have differing sampling frequencies: ',patinfo.chlabels{EEG_CHAN1}])
                    if patinfo.fs_orig(EEG_CHAN1) ~= patinfo.fs_orig(EEG_CHAN2)
                            data_signal1 = [data.(patinfo.chlabels{EEG_CHAN1})]';
                            data_signal1 = reshape(data_signal1,numel(data_signal1),1);
                            data_signal2 = [data.(patinfo.chlabels{EEG_CHAN2})]';
                            data_signal2 = reshape(data_signal2,numel(data_signal2),1);
                        if fs <= patinfo.fs_orig(EEG_CHAN1) %downsample
                            data_signal1 = resample(data_signal1,fs,round(patinfo.fs_orig(EEG_CHAN1))); 
                        else %upsample
                            disp(['Need to Upsample Signal: ',patinfo.chlabels{EEG_CHAN1}])
                            data_signal1 = resample(data_signal1,fs,round(patinfo.fs_orig(EEG_CHAN1)));                             
                        end
                        if fs <= patinfo.fs_orig(EEG_CHAN2)
                            data_signal2 = resample(data_signal2,fs,round(patinfo.fs_orig(EEG_CHAN2))); 
                        else
                            disp(['Need to Upsample Signal: ',patinfo.chlabels{EEG_CHAN1}])                            
                            data_signal2 = resample(data_signal2,fs,round(patinfo.fs_orig(EEG_CHAN2)));                                                         
                        end
                        data_signal = data_signal1-data_signal2;
                    else
                    data_signal = [data.(patinfo.chlabels{EEG_CHAN1}) - data.(patinfo.chlabels{EEG_CHAN2})]';   
                    end
                end


                if isempty(EEG_CHAN1)
                   EEG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'eeg'));   
                end
                if isempty(EEG_CHAN2) && isempty(EEG_CHAN1)
                    EEG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'C4M1'));                
                end
                if ~isempty(EEG_CHAN1) && isempty(EEG_CHAN2)
                    data_signal = [data.(patinfo.chlabels{EEG_CHAN1})]';                    
                end
...
%                 EOG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'EOG'));
%                 EOG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'eog'));
% 
...
%                    else
%                        disp(['Need to Upsample Signal: ',patinfo.chlabels{EOG_CHAN1}])
%                    end
%                 end 
%                 if filt_flag
%                     data_signal = filtfilt(eeg_band,1,data_signal);                    
...
%                
            ...
            case {'EOG'}
                ...
                EOG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'ROC'));
                EOG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'LOC'));                    
                end
                if isempty(EOG_CHAN2) && isempty(EOG_CHAN1)
                    EOG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'EOGl')); %Check if actually EOG?
                    EOG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'EOGr')); 
...
%                 data_signal = squeeze(data(:,:,EOG_CHAN))';
                if isempty(EOG_CHAN2) && isempty(EOG_CHAN1)
                    EOG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'E1M2')); %Check if actually EOG?
                    EOG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'E2M2')); 

                end
                if isempty(EOG_CHAN2) && isempty(EOG_CHAN1)
                    EOG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'X1')); %Check if actually EOG?
                    EOG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'X2')); 
                    data_signal = [data.X1 - data.X2]';
                end
                ...
                if ~isempty(EOG_CHAN2) && ~isempty(EOG_CHAN1)
                    data_signal = [data.(patinfo.chlabels{EOG_CHAN1}) - data.(patinfo.chlabels{EOG_CHAN2})]';                    
                end
                if isempty(EOG_CHAN2) && ~isempty(EOG_CHAN1)
                    data_signal = [data.(patinfo.chlabels{EOG_CHAN1})]';                    
                end
                

                data_signal = reshape(data_signal,numel(data_signal),1);
                if patinfo.fs_orig(EOG_CHAN1) ~= patinfo.fs_orig(EOG_CHAN2)
                       disp(['Mixe Sampling Frequencies: ',patinfo.chlabels{EOG_CHAN1}])
                   
                else
                   if  fs <= patinfo.fs_orig(EOG_CHAN2)
                       data_signal = resample(data_signal,fs,round(patinfo.fs_orig(EOG_CHAN2)));
                   else
                       disp(['Need to Upsample Signal: ',patinfo.chlabels{EOG_CHAN2}])
                       data_signal = resample(data_signal,fs,round(patinfo.fs_orig(EOG_CHAN2)));                       
                   end                    
                end 
                if filt_flag
            ...
            case {'EMG'}
                ...
                EMG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'CHIN'));
                EMG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'chin'));
                if isempty(EMG_CHAN1)
                EMG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'CHIN1'));
                EMG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'CHIN2'));                    
                end
               
                if isempty(EMG_CHAN1) && isempty(EMG_CHAN2)
                EMG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'X1')); %Check if actually EMG?
                EMG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'X2'));     
                end
                if isempty(EMG_CHAN1) && isempty(EMG_CHAN2)
                    EMG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'ChinL'));
                    EMG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'ChinR'));
                end
                if isempty(EMG_CHAN1) && isempty(EMG_CHAN2)
                EMG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'PLM1')); %Check if actually EMG?
                EMG_CHAN2 = find(strcmp(cellstr(patinfo.chlabels), 'PLM2'));     
                end

                if isempty(EMG_CHAN1) && isempty(EMG_CHAN2)
                    EMG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'emg'));
                end
                if isempty(EMG_CHAN1) && isempty(EMG_CHAN2)
                    EMG_CHAN1 = find(strcmp(cellstr(patinfo.chlabels), 'SEMG'));
                end          

                disp("Extracting EMG features")
%                 data_signal = squeeze(data(:,:,EMG_CHAN))';

                if ~isempty(EMG_CHAN2) && ~isempty(EMG_CHAN1)
                    data_signal = [data.(patinfo.chlabels{EMG_CHAN1}) - data.(patinfo.chlabels{EMG_CHAN2})]';                    
                end
                if isempty(EMG_CHAN2) && ~isempty(EMG_CHAN1)
                    data_signal = [data.(patinfo.chlabels{EMG_CHAN1})]';                    
                end
                ...
                if patinfo.fs_orig(EMG_CHAN1) == patinfo.fs_orig(EMG_CHAN2)
                   if  fs <= patinfo.fs_orig(EMG_CHAN1)
                       ...
                       disp(['Need to Upsample Signal: ',patinfo.chlabels{EMG_CHAN1}])
                       data_signal = resample(data_signal,fs,round(patinfo.fs_orig(EMG_CHAN1)));                       
                   end
                else
                   if  fs <= patinfo.fs_orig(EMG_CHAN1)
...
%                 [ecg_feats, features_struct_30s,ecg_data_signal] = FeatExtract_HR_mat7(data_signal, fs,epoch_time,std_flag,0,subject,dbpath,hyp); %Without RR averaging window
             ...
             case {'ECGasEMG'}
                ECG_CHAN = find(strcmp(cellstr(patinfo.chlabels), 'ECG'));
