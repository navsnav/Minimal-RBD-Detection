function [All_Sleep, Sleep_Struct, All_Sleep_T] = Sleep_Project_ExtractFeatures_mat(dbpath,signals_for_processing,std_flag)
...
for f=1:length(fls)
    ...
    hyp(R_idx) = 5;    
        ...
        hyp = zeros(floor(length(data.eeg)/data.patinfo.fs_orig(1,1)/30),1);
        skip_s = length(data.eeg)/data.patinfo.fs_orig(1,1) -30*length(hyp);
    end
...
%                 data_signal = squeeze(data(:,:,EEG_CHAN))';
                data_signal = data.eeg;
                data_signal = reshape(data_signal',numel(data_signal),1);
                if skip_s > 0
                    data_signal = data_signal(1:end-skip_s*data.patinfo.fs_orig(1,EEG_CHAN));
                end
                disp("Extracting EEG features")
                [eeg_feats, features_struct_30s,eeg_data_signal,~] = FeatExtract_EEG_mini2(data_signal, data.patinfo.fs_orig(1,EEG_CHAN),epoch_time,feature_time,0);
...
%                 data_signal = squeeze(data(:,:,EOG_CHAN))';
                ...
                data_signal = reshape(data_signal',numel(data_signal),1);
                if skip_s > 0
                    data_signal = data_signal(1:end-skip_s*data.patinfo.fs_orig(1,EOG_CHAN));
                end
                [eog_feats, features_struct_30s,eog_data_signal,~] = FeatExtract_EOG_mini2(data_signal, data.patinfo.fs_orig(1,EOG_CHAN),epoch_time,feature_time,0);
...
%                 data_signal = squeeze(data(:,:,EMG_CHAN))';
                data_signal = data.emg;                
                data_signal = reshape(data_signal',numel(data_signal),1);
                if skip_s > 0
                    data_signal = data_signal(1:end-skip_s*data.patinfo.fs_orig(1,EMG_CHAN));
                end
                [emg_feats, features_struct_30s, emg_data_signal,~] = FeatExtract_EMG_mini(data_signal, data.patinfo.fs_orig(1,EMG_CHAN),epoch_time, hyp,0);
            ...
            case {'EMG_L'}
                EMG_CHAN = find(strcmp(cellstr(patinfo.chlabels), char(signals_for_processing(j))));
                if isempty(EMG_CHAN)
                   EMG_CHAN = find(strcmp(cellstr(patinfo.chlabels),'EMG_l'));                
                end
                disp("Extracting EMG_L features")
%                 data_signal = squeeze(data(:,:,EMG_L_CHAN))';
                data_signal = data.emg_l;               
                data_signal = reshape(data_signal',numel(data_signal),1);
                if skip_s > 0
                    data_signal = data_signal(1:end-skip_s*data.patinfo.fs_orig(1,EMG_CHAN));
                end
                [emg_l_feats, features_struct_30s, emg_data_signal,~] = FeatExtract_EMG_mini(data_signal, data.patinfo.fs_orig(1,EMG_CHAN),epoch_time, hyp,0);
            ...
            case {'EEG-EOG'}
                ...
                feature_time = 10;
                if data.patinfo.fs_orig(1,EEG_CHAN) > data.patinfo.fs_orig(1,EOG_CHAN)
                    eeg_data_signal = resample(eeg_data_signal,data.patinfo.fs_orig(1,EOG_CHAN),data.patinfo.fs_orig(1,EEG_CHAN));
                    fs = data.patinfo.fs_orig(1,EOG_CHAN);
                else
                    eog_data_signal = resample(eog_data_signal,data.patinfo.fs_orig(1,EEG_CHAN),data.patinfo.fs_orig(1,EOG_CHAN));
                    fs = data.patinfo.fs_orig(1,EEG_CHAN);
                end
