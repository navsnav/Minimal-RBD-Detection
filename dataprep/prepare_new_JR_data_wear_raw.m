function prepare_new_JR_data_wear_raw(filename,outputfolder,std_flag,raw_flag)
...
cd(['Y:\RBD-Wearables\raw\RBD_study2018\']);
...
for f = 1:length(flsedf)
    ...
    if sum(idxrow) > 1
       all_dates = genderage.Date;
%        all_dates = datetime(strrep(all_dates,'/','.'),'InputFormat','dd.MM.yyyy'); 
       recday = datetime([hdr.startdate],'InputFormat','dd.MM.yy');
       date_m_idx = datefind(recday,all_dates);
       date_match_idx = zeros(size(idxrow)); 
       date_match_idx(date_m_idx) =1;
       idxrow = date_match_idx==1;       
    end
...
%     epoch = round(tstamp/patinfo.fs);
    ...
    %     close
    if raw_flag
        save([outputfolder,subject,'_raw.mat'],'data','patinfo')
    else
            save([outputfolder,subject,'.mat'],'data','patinfo')    
    end
